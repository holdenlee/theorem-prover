...
import SimpleWorkspaceM

newtype IVar = IVar Int deriving (Eq, Ord)
newtype String' = String' String deriving (Eq, Ord)

instance (Show String') where
    show (String' str) = str
--rather than show str
...
instance Applicative Monotype where
    pure x = JustMT x
    (JustMT f) <*> x = fmap f x --I'm confused about this.

instance Monad Monotype where
    (JustMT x) >>= f = f x
    (ApplyT str li) >>= f = ApplyT str (map (>>=f) li)

mapPT :: (c -> Monotype c) -> Polytype c -> Polytype c
mapPT f pt = case pt of
               JustPT y -> JustPT (y >>= f)
               Forall x y -> Forall x (mapPT f y)

{-
subM :: Setting (Polytype c) (Polytype d) c (Monotype d)
subM = setting (\f pt ->
                 case pt of
                   JustPT y -> JustPT (y >>= f)
                   Forall x y -> Forall x (sets f y))-}
...
mapToF :: (Ord a, Monad m) => M.Map a (m a) -> (a -> m a)
mapToF m x = case M.lookup x m of
               Nothing -> return x
               Just y -> y

subst :: (Ord c) => M.Map c (Monotype c) -> Polytype c -> Polytype c
subst m (JustPT x) = JustPT (x >>= (mapToF m))
subst m (Forall x y) = subst m y --kill all Forall's
{-
...
{-
addfree :: (Ord c) => [c] -> Polytype c -> Polytype c
addfree (h:rest) pt = if h `S.member` (freeP pt) then addfree rest pt else addfree rest $ Forall h pt
addfree [] pt = pt-}

addfree :: (Ord c) => [c] -> (S.Set c) -> Polytype c -> Polytype c
addfree (h:rest) s pt = if h `S.member` s then addfree rest s pt else addfree rest s $ Forall h pt
addfree [] s pt = pt

spec :: (Ord c) => (M.Map c (Monotype c)) -> Polytype c -> [c] -> Maybe (Predicate a b c)
spec m t li = Just $ Spec t (t & subst m & addfree li (freeP t))

x_ = IVar
a_ = IVar

gamma_0 :: Context String' IVar IVar
gamma_0 = (S.fromList [(JustE (JustA (String' "id"))) -:: (Forall (x_ 1) (JustPT (JustMT (x_ 1) --> (JustMT (x_ 1))))),
                   (JustE (JustA (String' "n"))) -:: (JustPT (ApplyT ("int") []))])
{-
...
proof = do
  eq1 <- var ((JustE (JustA (String' "id"))) -:: (Forall (x_ 1) (JustPT (JustMT (x_ 1) --> (JustMT (x_ 1)))))) gamma_0
  eq_spec <- spec (M.fromList [(x_ 1, ApplyT ("int") [])]) (Forall (x_ 1) (JustPT (JustMT (x_ 1) --> (JustMT (x_ 1))))) []
  eq2 <- inst eq1 eq_spec
  eq3 <- var ((JustE (JustA (String' "n"))) -:: (JustPT (ApplyT ("int") []))) gamma_0
  eq4 <- app eq2 eq3
  return eq4
